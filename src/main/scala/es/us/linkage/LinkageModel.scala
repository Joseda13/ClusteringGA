...
class LinkageModel(_clusters: RDD[(Long, (Int, Int))], var _clusterCenters: Array[Vector]) extends Serializable {
  ...
  def createClusters(numPoints: Int, numCluster: Int, totalPoints: RDD[Int]): RDD[(Int, Int)] = {
    ...
    println("Time for create result model: " + duration)

    //Return the result of clustering
    auxPoints
  }

  def createContingencyTable (numPoints: Int, numCluster: Int, data: DataFrame, kMin: Int, numStaticCluster: Int): DataFrame = {
    val spark = data.sparkSession
    import spark.implicits._

    val totalPoints = spark.sparkContext.parallelize(1 to numPoints).cache()
    var res = totalPoints.toDF()
    ...
    val dataRDD = data.rdd.map(row => (row.getLong(1).toInt,row.getLong(1).toInt))
    val joinRDDs = dataRDD.join(auxPoints).map(value => (value._2._2,value._2._1)).groupByKey()
    val joinRDDsFiltered = joinRDDs.filter(_._2.size >= kMin)
//    joinRDDsFiltered.collect.foreach(println(_))
//    println("numero de clusters: " + joinRDDsFiltered.count())

    if(joinRDDsFiltered.count() >= numStaticCluster){
//      println("1")
      val auxPointsDF = auxPoints.toDF().withColumnRenamed("_1","index")
      val joinDFs = data.join(auxPointsDF, data.col("index") === auxPointsDF.col("index")).drop("index")
//      joinDFs.show(30)
      res = joinDFs.withColumnRenamed("_2","prediction")
    }
    //If not, the clusters of the following number of clusters are calculated
    else if (numCluster < (numPoints - 5)){
//      println("2")
      val joinRDDsEliminated = joinRDDs.subtract(joinRDDsFiltered)

      if(joinRDDsEliminated.count() >= 1){
//        println("3")
        val f1 = joinRDDsEliminated.map{ value =>
          value._2.toSeq
        }.reduce((a,b) => a ++ b)

        val dataWithElementEliminated = data.filter(!$"index".isin(f1: _*)).select("class","index")
//        res = createContingencyTable(numPoints, numCluster + 1, dataWithElementEliminated, Math.round((kMin.toFloat/numPoints)*joinRDDs.map(_._2.size).max()), numStaticCluster)
        res = createContingencyTable(numPoints, numCluster + 1, dataWithElementEliminated, kMin, numStaticCluster)
      } else{
//        println("4")
        res = createContingencyTable(numPoints, numCluster + 1, data, kMin, numStaticCluster)
//        res = createContingencyTable(numPoints, numCluster + 1, data, Math.round((kMin.toFloat/numPoints)*joinRDDs.map(_._2.size).max()), numStaticCluster)
      }
    }
...
//      Math.round((kMin.toFloat/numPoints)*numPoints)
      rest = inicializeCenters(coordinates, Math.round((kMin.toFloat/numPoints)*numPoints), numPoints, 2, totalPoints, 2)
    }

    //Show the duration to create the centroids
    val duration = (System.nanoTime - start) / 1e9d
    println("Time for create centroids: " + duration)
