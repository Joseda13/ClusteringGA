...
class Linkage(
  ...
  /**
    ...
    */
  def runAlgorithm(distanceMatrix: RDD[Distance], numPoints: Int, checkMatrix: Int, checkTotal: Int): LinkageModel = {
    ...
    for (a <- 0 until (numPoints - numClusters)) {
      ...
      val durationMin = (System.nanoTime - startIter) / 1e9d
      println("Time for found min cluster: " + durationMin)
      ...
      val durationModel = (System.nanoTime - startIter) / 1e9d
      println("Time for update the model: " + durationModel)
      ...
      val durationCount = (System.nanoTime - startIter) / 1e9d
      println("Time for counts size points: " + durationCount)
      ...
      val durationUpdateTotalPoints = (System.nanoTime - startIter) / 1e9d
      println("Time for update totalPoints: " + durationUpdateTotalPoints)

      //If it isn´t the last cluster
      if (a < (numPoints - numClusters - 1)) {
        val startIf = System.nanoTime
        //The point found is deleted
        matrix = matrix.filter(x => !(x.getIdW1 == point1 && x.getIdW2 == point2))
        val durationFilter = (System.nanoTime - startIf) / 1e9d
        println("Time for filter matrix: " + durationFilter)
        ...
        val durationCollect1 = (System.nanoTime - startIf) / 1e9d
        println("Time for first collect: " + durationCollect1)
        ...
        val durationCollect2 = (System.nanoTime - startIf) / 1e9d
        println("Time for second collect: " + durationCollect2)
        ...
        val durationCreateSub = (System.nanoTime - startIf) / 1e9d
        println("Time for create matrixSub: " + durationCreateSub)
        ...
        val durationCartesian = (System.nanoTime - startIf) / 1e9d
        println("Time for cartesian RDD: " + durationCartesian)
        ...
        matrix = distanceStrategy match {
          ...
          case "avg" =>
            ...
            val durationNewPoints = (System.nanoTime - startAVG) / 1e9d
            println("Time for create newPoints: " + durationNewPoints)
            //Add the points with the new index
            matrixSub.union(newPoints)
        }
      }
      ...
      val durationCoalesce = (System.nanoTime - startIter) / 1e9d
      println("Time for coleasce: " + durationCoalesce)
      //Every 5 iterations a checkpoint of the distance matrix is ​​done to improve the performance of the algorithm
      if (a % checkMatrix == 0) {
        matrix.checkpoint()
        val durationCheckMatrix = (System.nanoTime - startIter) / 1e9d
        println("Time for checkpoint matrix: " + durationCheckMatrix)
      }
      ...
      if (a % checkTotal == 0){
        ...
        val durationTotalPoints = (System.nanoTime - startIter) / 1e9d
        println("Time for checkpoint totalPoints: " + durationTotalPoints)
      }
    ...
    println("Time for linkage clustering: " + duration)
