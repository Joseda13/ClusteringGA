...
object MainTestChangeNV {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val arguments = List(
      ...
      Array[String](pathDataSets+"K5-N20-D10-DES0_03", "30")
    )

    val baseTest = pathDataSets + "K5-N20-D20-DES0_03"
    val dimension = 40
    val K = 5
    ...
    for (index <- 0 to 100) {
      ...
      for (in <- 0 to 4) {
//        chromoTest.inicializeChromosome()
        chromoTest.inicializeChromosomeWithDummiesRandom(in)
//        chromoTest.inicializeChromosome()
        println(chromoTest.contAttributeGood() + ";" + chromoTest.contDummies() + " => " + Indices.getFitnessSilhouette(chromoTest.getGenes, baseTest))
      }
